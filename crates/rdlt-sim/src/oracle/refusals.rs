...
pub(super) struct Prediction {
    ...
    /// The streams every run of which meets one, so no run loading them succeeds.
    pub(super) must: BTreeSet<String>,
}
    ...
    for (stream, relaxed) in world.workload.streams.iter().zip(relaxed) {
        ...
        for outcome in columns.chain(key) {
            if let Some(code) = outcome.code {
                ...
                if outcome.must && !pruning {
                    prediction.must.insert(stream.name.clone());
                }
            }
        }
    }
