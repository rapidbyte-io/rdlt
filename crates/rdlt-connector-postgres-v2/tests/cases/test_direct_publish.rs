...
async fn a_redelivered_unit_never_duplicates_its_rows() {
    ...
    let destination =
        rdlt_connector_postgres_v2::destination::Postgres::new(&connection_string).schema("rp1");
...
async fn a_replace_target_first_written_in_unit_two_is_still_cleared() {
    ...
    let connection_string = container.connection_string.clone();
    let destination =
        rdlt_connector_postgres_v2::destination::Postgres::new(&connection_string).schema("rp2");
