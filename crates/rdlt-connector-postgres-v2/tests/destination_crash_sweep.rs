...
fn registry_is_pinned() {
    let mut registry: Vec<&str> = rdlt_connector_postgres_v2::destination::FAIL_POINTS.to_vec();
...
fn reachable(mode: &str) -> Vec<&'static str> {
    rdlt_connector_postgres_v2::destination::FAIL_POINTS
        .iter()
        .copied()
...
async fn every_dest_fail_point_recovers_exactly_once_across_append_replace_and_merge() {
    ...
    for &point in rdlt_connector_postgres_v2::destination::FAIL_POINTS {
...
fn with_strategy(
    destination: Postgres,
    strategy: rdlt_connector_postgres_v2::destination::MergeStrategy,
) -> Postgres {
    destination
        .options(
            rdlt_connector_postgres_v2::destination::DestinationOptions {
...
async fn every_dest_fail_point_recovers_exactly_once_under_keyed_structured_merge() {
    ...
    let strategies = [
        (
            "di",
            rdlt_connector_postgres_v2::destination::MergeStrategy::DeleteInsert,
        ),
        (
            "up",
            rdlt_connector_postgres_v2::destination::MergeStrategy::Upsert,
        ),
    ];
    let mut fired: std::collections::BTreeSet<(&str, &str)> = std::collections::BTreeSet::new();
    for &point in rdlt_connector_postgres_v2::destination::FAIL_POINTS {
            ...
            let destination = Postgres::new(&connection_string)
                ...
                .options(
                    rdlt_connector_postgres_v2::destination::DestinationOptions {
                        tables: [(
                            "s".to_string(),
                            rdlt_connector_postgres_v2::destination::TableOptions {
                                dedup_sort: Some(
                                    rdlt_connector_postgres_v2::destination::DedupSort {
                                        column: "seq".into(),
                                        order:
                                            rdlt_connector_postgres_v2::destination::SortOrder::Desc,
                                    },
                                ),
...
fn the_registry_matches_the_sources() {
    ...
    rdlt_testkit::assert_registry_matches_sources(
        ...
        &[
            rdlt_connector_postgres_v2::destination::FAIL_POINTS,
            rdlt_connector_postgres_v2::source::FAIL_POINTS,
            rdlt_connector_postgres_v2::source::CDC_FAIL_POINTS,
        ],
    );
}
