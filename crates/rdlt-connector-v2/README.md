...
> **This crate is not published and has no in-tree consumer yet.** The
> engine and every connector still build against the first-generation
> crate, `rdlt-connector`. Behavior is contract-identical: the six error
> classification frames, the channel's byte-budget semantics, secret
> redaction, and every serde spelling are frozen surfaces carried over
> verbatim. The swap — porting the engine and connectors — is the owner's
> call, exactly as it was for the postgres and rest second generations.
> The design record is `specs/027-sdk-trio/plan.md`.
