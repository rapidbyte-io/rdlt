//! Locating the built `rdlt-connector-oracle` bin — the 039
//! rdlt-runtime pattern, replicated: `CARGO_TARGET_DIR` honored the
//! way rdlt-bench honors it for the CLI, the build itself guarded by
...
fn workspace_root() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR"))
        .ancestors()
        .nth(2)
        .expect("crates/rdlt-connector-oracle sits two levels below the workspace root")
        .to_path_buf()
}
...
/// The path to the built `rdlt-connector-oracle` bin, building it
    ...
    BUILT.get_or_init(|| {
        if std::env::var_os("RDLT_BUILD_CONNECTOR_BINS").is_none() {
            ...
            eprintln!(
                "note: RDLT_BUILD_CONNECTOR_BINS is unset — spawning the \
                 rdlt-connector-oracle binary already on disk WITHOUT rebuilding. \
        ...
        let status = std::process::Command::new(&cargo)
            ...
            .unwrap_or_else(|error| {
                panic!("cargo build -p rdlt-connector-oracle did not spawn: {error}")
            });
        assert!(
            status.success(),
            "cargo build -p rdlt-connector-oracle --features bin-serve failed"
        );
    });
    let path = target_debug_dir().join("rdlt-connector-oracle");
    assert!(
        path.is_file(),
        "connector binary `{}` is not built — run the Makefile's spawn-bins \
         line (it sets RDLT_BUILD_CONNECTOR_BINS=1) or `cargo build -p \
         rdlt-connector-oracle --features bin-serve` first",
        path.display()
    );
    path
}
