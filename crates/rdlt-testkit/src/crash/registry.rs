//! The crash-point registry scanner: proof that a declared registry and the
//! sites armed in the sources agree. Three earlier designs were overturned by
//! measurement and each wrong one FAILS OPEN — do not simplify this scanner
//! (set equality, occurrence counting, and one-assertion-per-registry are the
//! recorded wrong designs; the module docs below carry the evidence).
