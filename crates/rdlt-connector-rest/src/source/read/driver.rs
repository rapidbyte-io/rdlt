...
//! (`rest.request`, `rest.decode`). The state machine every read path drives,
//! parentless or fanned-out alike.
...
use crate::source::client::{self, RestClient};
use crate::source::config::{
    ActionKind, HttpMethod, Incremental, ResponseAction, RestConfig, RestStream,
};

use super::extract::{Extracted, Selector, extract_records};
use super::paginate::{PageContext, PageDecision, Paginator};
use super::resolve::{self, ParentValues};

/// One paginated request sequence: request build, actions, guards.
pub(crate) struct SequenceDriver<'a> {
    config: &'a RestConfig,
    stream: &'a RestStream,
    client: &'a RestClient,
...
impl<'a> SequenceDriver<'a> {
    ...
    /// sequence: the resume ("since") param and any closed-window upper bound.
    pub(crate) fn bind_incremental(
        &mut self,
        incremental: &Option<Incremental>,
        start_value: &Option<String>,
    ) {
        let Some(inc) = incremental else { return };
    ...
    pub(crate) async fn fetch_page(
        ...
        if !status.is_success() {
            return Err(client::classify_status(status, retry_after));
        }
        ...
        self.last_body = if self.paginator.needs_body() {
            Some(super::extract::parse_body(&body)?)
        } else {
            None
        };
        Ok(Some(extracted))
    ...
    pub(crate) fn advance(&mut self, extracted: &Extracted) -> Result<bool, SourceError> {
        if self.done {
            return Ok(false);
        }
        let ctx = PageContext {
...
/// paginator's page params ride: query for GET (and body-less POST), or into
/// the JSON body for a POST-cursor API. Source/stream params and incremental
/// bindings ride as query in every case; parent placeholders are resolved
/// against `parent_values` when this is a child sequence.
fn build_page_request(
    ...
    match (stream.method, &stream.body) {
        (HttpMethod::Post, Some(body)) => {
            // Pagination params go INTO the body for POST-cursor APIs (R5).
