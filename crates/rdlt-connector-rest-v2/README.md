# rdlt-connector-rest-v2

Declarative REST source, second generation: one YAML/JSON document
describes an API — auth, pagination, record selection, incremental
cursors, response handling, parent-child stream composition — and the
connector reads it into the engine with typed errors for everything that
can go wrong. It is also a LIBRARY: named-API connectors (a
Google-Search-Console-style wrapper) are built from the same public
pieces, inheriting the client, the paginators, and the whole test
discipline instead of re-implementing HTTP.

> **This crate is not published and not wired into the `rdlt` facade.**
> `rdlt::connector::rest` and the CLI's `rest:` blocks still resolve to
> the first-generation crate, `rdlt-connector-rest`. Behavior, the config
> document vocabulary, error classification, and operational semantics
> here are identical to that crate; what is new is the Rust API — module
> paths, type names, and internal structure. Swapping the facade over is
> the owner's decision. The design record is `specs/026-rest-v2/plan.md`.
