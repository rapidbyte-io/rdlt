...
async fn parent_validation_matrix() {
    for (yaml_frag, needle) in [
        ...
        let err = rdlt_connector_rest_v2::source::Config::from_yaml(&yaml)
            .expect_err(needle)
            .to_string();
...
async fn zero_max_concurrency_rejected_at_parse() {
    let err = rdlt_connector_rest_v2::source::Config::from_yaml(
        "base_url: http://x\nmax_concurrency: 0\nstreams:\n  - name: a\n    path: /a\n",
    )
...
mod composed_api {
    use rdlt_connector_rest_v2::source::{Config, Rest};
