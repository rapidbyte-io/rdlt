...
async fn invalid_selector_fails_at_parse() {
    let err = rdlt_connector_rest_v2::source::Config::from_yaml(&stream_yaml(
        "http://x",
        "a",
        "/a",
...
async fn validation_matrix_covers_remaining_arms() {
    ...
    rdlt_connector_rest_v2::source::Config::from_json(
        r#"{"base_url": "http://x", "streams": [{"name": "a", "path": "/a"}]}"#,
    )
    .expect("valid JSON document parses");
    let err = rdlt_connector_rest_v2::source::Config::from_json(
