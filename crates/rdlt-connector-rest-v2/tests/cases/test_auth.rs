...
async fn pre_014_tagged_auth_spelling_parses_and_attaches() {
    for yaml in [
        ...
        rdlt_connector_rest_v2::source::Config::from_yaml(yaml)
...
async fn secrets_never_render_anywhere() {
    ...
    for auth in [
        ...
        let config = rdlt_connector_rest_v2::source::Config::from_yaml(&yaml).expect("parses");
        let rendered_config = format!("{config:?}");
        let source = rdlt_connector_rest_v2::source::Rest::new(config).expect("valid config");
