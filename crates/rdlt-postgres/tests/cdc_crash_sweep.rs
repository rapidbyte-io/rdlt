...
fn cdc_registry_is_pinned() {
    let mut registry: Vec<&str> = rdlt_postgres::source::CDC_FAIL_POINTS.to_vec();
...
async fn sweep_cdc_fail_points() {
    ...
    for &point in rdlt_postgres::source::CDC_FAIL_POINTS {
...
async fn container_kill_mid_catch_up_is_typed_and_preserves_commits() {
    ...
    let dest = rdlt_dest_duckdb::DuckDb::open(dir.path().join("out.duckdb")).expect("open db");
    let workdir = dir.path().join("wal");
    std::mem::forget(dir);
    let run = |src_conn: String, dest: rdlt_dest_duckdb::DuckDb, workdir: std::path::PathBuf| async move {
