...
fn schema(t: &str) -> TableSchema {
    TableSchema {
        ...
        columns: vec![ColumnDef {
            name: "id".into(),
            column_type: ColumnType::Scalar { scalar: LogicalType::Int64 },
            nullable: false,
...
async fn count(conn: &str, t: &str) -> i64 {
    let (c, connection) = tokio_postgres::connect(conn, tokio_postgres::NoTls).await.expect("conn");
    tokio::spawn(async move { let _ = connection.await; });
    c.query_one(&format!("SELECT count(*) FROM {t}"), &[]).await.expect("count").get(0)
}
...
async fn probe_redelivered_unit_does_not_duplicate() {
    ...
    let meta = |seq: u64| CommitMeta {
        load_id: LoadId::new("rp1-load"), commit_seq: seq,
        state: StateDoc::new(pipeline.clone(), env!("CARGO_PKG_VERSION")),
        counters: CommitCounters::default(),
    };
    let mut s = dest.open(OpenCtx::new(pipeline.clone(), LoadId::new("rp1-load"))).await.expect("open");
    s.ensure_table(&schema("t"), &WriteMode::Append).await.expect("ensure");
    s.write(&TableName::new("t"), batch(&[1, 2, 3])).await.expect("write");
    ...
    s.write(&TableName::new("t"), batch(&[1, 2, 3])).await.expect("re-write");
...
async fn probe_replace_first_written_in_unit_two_is_cleared() {
    ...
    let mk = |load: &str, seq: u64| CommitMeta {
        load_id: LoadId::new(load), commit_seq: seq,
        state: StateDoc::new(pipeline.clone(), env!("CARGO_PKG_VERSION")),
        counters: CommitCounters::default(),
    };
    // Load 1 leaves rows behind.
    let mut s = dest.open(OpenCtx::new(pipeline.clone(), LoadId::new("L1"))).await.expect("open");
    s.ensure_table(&schema("t"), &WriteMode::Replace).await.expect("ensure");
    s.write(&TableName::new("t"), batch(&[1, 2, 3])).await.expect("write");
    ...
    let mut s = dest.open(OpenCtx::new(pipeline.clone(), LoadId::new("L2"))).await.expect("open");
    s.ensure_table(&schema("t"), &WriteMode::Replace).await.expect("ensure");
    s.commit(mk("L2", 0)).await.expect("empty unit commits");
    s.write(&TableName::new("t"), batch(&[9])).await.expect("write in unit 1");
    s.commit(mk("L2", 1)).await.expect("commit unit 1");
    assert_eq!(count(&conn, "rp2.t").await, 1, "REPLACE MUST CLEAR load 1's rows");
}
