...
- **0** — every clause passed (skips are honest non-verdicts, not
  ...
  exit 1 unless its stream is named in `--accept-skips
  <stream[,stream]>` (the library's `accept_skips` stream list),
  because a source that never checkpoints looks identical to one that
...
| P13 | unserved-role refusal | Spawning the binary with a --role it does not serve must exit with code 2 before writing any stdout byte — the handshake line is written only for a served role, and the role-less schema probe relies on exactly this refusal signal. A connector serving both roles has no unserved role, so the clause is skipped with the reason naming both. |
