...
|---|---|---|---|
| CSV inference lattice | join over the WHOLE file (int→float widening; bool×numeric and anything×text join to utf8 — review F3); empty = null; non-finite floats typed naming file/row/column (review F10) | csv.rs::inference_lattice_and_nulls + bool_and_numeric_column_widens_to_text + non_finite_inferred_float_is_typed | local |
...
| tail-hash resume integrity (review F2) | a GROWN file whose pre-resume bytes changed fails typed on both kinds (blake3 of the last consumed window verified before any tail read); genuine appends still resume (the delta cells) | jsonl.rs::grown_rewrite_fails_via_tail_hash; s3_live.rs::grown_object_rewrite_is_typed_by_tail_hash; jsonl.rs::resume_reads_only_appended_tail_and_new_files; s3_live.rs::seeded_bucket_loads_and_delta_runs_through_the_engine | local+live |
...
| (frozen `parquet:` spelling) | — | `ParquetDir::open(dir)` ≡ local parquet (PathBuf used AS-IS — no lossy round-trip); part naming, `_rdlt_state`/`_rdlt_commits` scope-hashed names, commit-log shape, D3 receipt dedup from PLANTED pre-015 bytes; Replace truncation keeps the EXACT pre-015 rule (top-level *.parquet) while new configs delete only their own part files (review F1) | — | preservation.rs::pre_015_artifact_names_and_commit_log_shape_are_frozen + pre_015_commit_log_fixture_drives_receipt_dedup; recovery.rs (all three); dest_conformance.rs (all); dest_options.rs::replace_truncation_spares_user_files | local |
