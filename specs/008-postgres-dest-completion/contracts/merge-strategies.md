...
| M4 | `hard_delete: <column>` (per table, delete-insert or upsert — not scd2): rows whose flag is set (boolean → `= TRUE`, other types → `IS NOT NULL`) DELETE their key at the destination and are EXCLUDED from insertion; unflagged rows merge normally; deleting a never-loaded key is a no-op. Column existence and shape validate at ensure time, errors naming the column. |
...
| M7 | Keyless streams: upsert is rejected typed at ensure time (a key is the whole point); delete-insert keeps its existing identity-based behavior for shredded streams. |
