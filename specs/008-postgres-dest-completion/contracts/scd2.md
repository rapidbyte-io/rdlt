...
| S6 | Absence policy: `keep` (default) — keys absent from the load keep their active version (incremental feeds are partial). `retire` — active keys absent from the stage retire at the boundary (full-feed semantics); the choice is per-table config. |
