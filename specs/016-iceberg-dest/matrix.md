...
| `tables.<stream>.partition_by` | `[]` | identity + temporal + bucket(N)/truncate(W) transforms → partition spec at CREATE (field-ids 1000+, spec-id explicit — Polaris parses strictly, probed live); fanout writes = one data file per partition value per commit (bucket bounds the fanout); spec visible in raw metadata AND to pyiceberg | unknown column typed at ensure; unknown transform cannot parse (closed enum); bucket/truncate zero parameter typed eagerly; spec FIXED at creation — live-table mismatch typed | src/dest/schema.rs::partition_tests (identity_and_temporal_transforms_build + bucket_and_truncate_transforms_build + empty_partition_by_is_none + unknown_partition_column_is_typed); config_schema.rs::bucket_and_truncate_spellings_and_validation; partitioning.rs::partitioned_writes_fan_out_and_spec_is_visible + bucket_and_truncate_partition_live + partition_spec_mismatch_is_typed; interop.rs::pyiceberg_reads_partitioned | unit+live |
