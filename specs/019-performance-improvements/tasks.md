...
- [ ] T076 [US7] Evaluate S3 unsigned payloads as a separable opt-in: `AmazonS3Builder::with_unsigned_payload(bool)` exists (`object_store-0.12.5/src/aws/builder.rs:863`, wired at `:1159`). Expose it explicitly — never as a default — and measure its effect against the recorded 6.72% `ring` SHA-256 share
