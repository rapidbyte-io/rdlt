...
pub struct Connection {
    ...
    closed: AtomicBool,
    id: u32,
    /// rdlt patch (032): rows requested per round trip. The wire
    /// field is a ub4, so this is a real throughput knob; 100 stays
    /// the default for compatibility with callers that expect the
    /// published behavior.
    default_prefetch_rows: u32,
}
...
impl Connection {
    ...
    fn parse_query_response_with_columns(
        ...
        let mut end_of_response = false;
        // rdlt patch (032): set when the server sends ORA-01403.
        ...
        while !end_of_response && buf.remaining() > 0 {
            ...
            match msg_type {
                ...
                x if x == MessageType::Error as u8 => {
                    ...
                    // rdlt patch (032): ORA-01403 is the server saying
                    // the cursor is EXHAUSTED. Its absence means rows
                    // remain — the published 0.1.7 hardcoded
                    // `has_more_rows: false` and so reported every
                    // truncated batch as complete (silent data loss).
                    exhausted = error_code == 1403;
                    end_of_response = true;
                }
        ...
        Ok(QueryResult {
            ...
            rows,
            rows_affected: row_count,
            // rdlt patch (032): honest continuation signal — see the
            // ORA-01403 note above.
            has_more_rows: !exhausted,
            cursor_id,
        })
    }
