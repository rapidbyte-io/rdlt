...
//!
//! Every connector that talks to an object store answers the same
//! question about the same error type — is this worth another attempt? —
//! and the answer must not depend on which connector asks. One copy lives
//! here; a second would drift, and the drift would surface as one
//! connector burning its retry budget on a certainty while another
//! reported the true cause.
