...
|---|---|---|---|---|---|---|---|
| pg-to-pg-1m | 749.8 ms (±5%) | **13.7×** (dlt: 10.26 s); 23.3× (dlt-pyarrow: 17.46 s); airbyte: MISSING (prerequisite failed for `airbyte`: abctl cluster unreachable (kubectl get ns airbyte-abctl)) | ≥ 4× | PASS | 1333634 | 254.0 | 104 MB |
| pg-to-s3parquet-1m | 937.4 ms (±2%) | **1.8×** (dlt: 1.69 s); 11.7× (dlt-pyarrow: 10.92 s); airbyte: MISSING (prerequisite failed for `airbyte`: abctl cluster unreachable (kubectl get ns airbyte-abctl)) | — | — | 1066794 | 203.2 | 107 MB |
| s3jsonl-to-pg-200k | 649.9 ms (±3%) | **101.1×** (dlt: 65.71 s); airbyte: MISSING (prerequisite failed for `airbyte`: abctl cluster unreachable (kubectl get ns airbyte-abctl)) | ≥ 40× | PASS | 923154 | 220.0 | 170 MB |
| s3jsonl-to-s3parquet-200k | 872.4 ms (±4%) | **69.1×** (dlt: 60.29 s); airbyte: MISSING (prerequisite failed for `airbyte`: abctl cluster unreachable (kubectl get ns airbyte-abctl)) | ≥ 45× | PASS | 687764 | 163.9 | 209 MB |
| pg-to-pg-dedup-1m | 4.39 s (±3%) | **2.9×** (dlt: 12.54 s); 4.7× (dlt-pyarrow: 20.81 s); airbyte: MISSING (prerequisite failed for `airbyte`: abctl cluster unreachable (kubectl get ns airbyte-abctl)) | ≥ 2× | PASS | 227659 | 43.2 | 100 MB |

_Generated by `rdlt-bench report` from committed artifacts (recorded 2026-07-28; airbyte 2.1.1, dlt 1.29.0)._
...
| pg-to-pg-1m | dlt | 10.26 s | 10.26 s | +0.0% |
| pg-to-pg-1m | dlt-pyarrow | 17.46 s | 17.32 s | +0.8% |
| pg-to-pg-1m | rdlt | 749.8 ms | 778.8 ms | -3.7% |
| pg-to-pg-dedup-1m | airbyte | 45.38 s | 45.39 s | -0.0% |
| pg-to-pg-dedup-1m | dlt | 12.54 s | 12.76 s | -1.7% |
| pg-to-pg-dedup-1m | dlt-pyarrow | 20.81 s | 20.57 s | +1.1% |
| pg-to-pg-dedup-1m | rdlt | 4.39 s | 4.82 s | -8.9% |
| pg-to-s3parquet-1m | airbyte | 45.37 s | 45.39 s | -0.0% |
| pg-to-s3parquet-1m | dlt | 1.69 s | 1.68 s | +0.3% |
| pg-to-s3parquet-1m | dlt-pyarrow | 10.92 s | 10.86 s | +0.6% |
| pg-to-s3parquet-1m | rdlt | 937.4 ms | 999.4 ms | -6.2% |
| s3jsonl-to-pg-200k | airbyte | 45.38 s | 45.37 s | +0.0% |
| s3jsonl-to-pg-200k | dlt | 65.71 s | 63.16 s | +4.0% |
| s3jsonl-to-pg-200k | rdlt | 649.9 ms | 665.2 ms | -2.3% |
| s3jsonl-to-s3parquet-200k | airbyte | 45.38 s | 45.37 s | +0.0% |
| s3jsonl-to-s3parquet-200k | dlt | 60.29 s | 58.11 s | +3.8% |
| s3jsonl-to-s3parquet-200k | rdlt | 872.4 ms | 914.1 ms | -4.6% |
| selftest-protocol | rdlt | 21.3 ms | 21.8 ms | -2.3% |
